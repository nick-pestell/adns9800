...
#include <avr/pgmspace.h>
	...
	const unsigned char firmware_data[] PROGMEM = {0x03, 0xa4, 0x6e, 0x16, 0x6d, 0x89, 0x3e, 0xfe, 0x5f, 0x1c, 0xb8, 0xf2, 0x47, 0x0c, 0x7b, 
...
adns9800::adns9800(int resetPin, int slaveSelectPin){
...
// power up routine
int adns9800::powerUp(){

	digitalWrite(_slaveSelectPin, HIGH);
	digitalWrite(_slaveSelectPin, LOW);
	
	writeTo(POWER_UP_RESET, 0x5a);
	delay(10);
	
	//burst_data data;
	//burstRead(&data);
	Serial.println(readFrom(0x02,1));
	readFrom(0x03,1);
	readFrom(0x04,1);
	readFrom(0x05,1);
	readFrom(0x06,1);
	...
	byte laser_control = readFrom(LASER_CTRL0,1);
	laser_control ^= 0xFF;
	laser_control |= 0x80;
	laser_control ^= 0xFF;
	writeTo(LASER_CTRL0,laser_control);
	
	//check connection
	digitalWrite(_slaveSelectPin, LOW);
	int ID = (int)readFrom(PROD_ID,1);
	delay(50);
...
// upload firmware
void adns9800::srom_upload(){
	
	byte srom_size = readFrom(CONFIG_IV,1);
	srom_size |= 0x02;
	writeTo(CONFIG_IV,srom_size);
	writeTo(SROM_ENABLE,0x1d);
	delay(0.5);
	writeTo(SROM_ENABLE,0x18);
	
	SPI.transfer(SROM_LOAD_BURST | WRITE);
	unsigned char c;
...
// send command to adns9800
void adns9800::writeTo(byte reg, byte value){
...
// recieve data from adns9800
byte adns9800::readFrom(byte reg, int byteNo){
...
// burst read motion data from adns9800
void adns9800::burstRead(struct burst_data *p){
  
  //Serial.println("in burst read");
  
  // make address with reg and read bit
  byte address =  MOTION_BURST & 0x7f;
  ...
  p->observation = SPI.transfer(0x00);
  p->dx_l =  SPI.transfer(0x00);
  p->dx_h =  SPI.transfer(0x00);
  p->dy_l =  SPI.transfer(0x00);
  p->dy_h =  SPI.transfer(0x00);
  p->squal = SPI.transfer(0x00);
  p->pixel_sum = SPI.transfer(0x00);
  p->max_pix = SPI.transfer(0x00);
  p->min_pix = SPI.transfer(0x00);
  p->shutter =  SPI.transfer(0x00)<<8;
  p->shutter |=  SPI.transfer(0x00);
  p->frame_period =  SPI.transfer(0x00)<<8;
  p->frame_period |=  SPI.transfer(0x00);
...
// capture an image
void adns9800::captureFrame(byte pdata[][30]){ 

  // write to frame capture register to prepare frame
  writeTo(FRAME_CAPTURE,0x83);

  // make address with reg and read bit
  byte address =  PIXEL_BURST & 0x7f;
...
int adns9800::convTwosComp(byte b){
